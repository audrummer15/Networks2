...
if {$argc != 6} {
  ...
  puts "usage:  ns exp.tcl Bw(kbps) Tp(ms) D(ms) N Protocol StartTimeInterval(ms)"
  exit 0
}
...
set StartTimeInterval [lindex $argv 5]
set StartTimeInterval [expr $StartTimeInterval / 1000.0];

puts "Bw = $Bw   Tp= $Tp    D = $D    N = $N  Protocol = $P   Start Time Interval = $StartTimeInterval"
...
# ------- TCP Connections ------- #
for {set i 1} {$i <= $N} {incr i} {
    set tcp($i) [$ns create-connection $P $s($i) TCPSink $r($i) 0]
